		...
		if  ( range > 1) 
				...
				if ( trans_info12_11[row] !=0 )
					...
					for ( col = 0 ; col < range ; col++)
						...
						//fprintf( stderr, "ext_info11( %d,%d ) = %lf \t", row ,col , ext_info11[ index_2D_11 ] ) ;
						aPosteriori1[ index_1D_11 ] += ext_info11[ index_2D_11 ] ;
							}
						}
					else
						{
			...
			{
			uint16_t col = 0 ;
			ext_info11[ index_2D_11 ] = trans_info12_11[row] ; 
			//fprintf( stderr, "ext_info11( %d) = %lf \t", row  , ext_info11[ index_2D_11 ] ) ;
			aPosteriori1[ index_1D_11 ] += ext_info11[ index_2D_11  ] ;
			}
		}
		...
		if  ( range > 1) 
				...
				if ( trans_info11_12[row] !=0 )
					...
					for ( col = 0 ; col < range ; col++)
						...
						//fprintf( stderr, "ext_info12( %d,%d ) = %lf \t", row ,col , ext_info12[ index_2D_12 ] ) ;
						aPosteriori1[ index_1D_12 ] += ext_info12[ index_2D_12 ] ;
							}
						}
					else
						{
			...
			{
			uint16_t col = 0 ;
			ext_info12[ index_2D_12 ] = trans_info11_12[row] ; 
			//fprintf( stderr, "ext_info12( %d) = %lf \t", row  , ext_info12[ index_2D_12 ] ) ;
			aPosteriori1[ index_1D_12 ] += ext_info12[ index_2D_12  ] ;
			}
		}
		...
		if  ( range > 1) 
				...
				if ( trans_info21_22[row] !=0 )
					...
					for ( col = 0 ; col < range ; col++)
						...
						//fprintf( stderr,"ext_info22( %d,%d ) = %lf \t", row ,col , ext_info22[ index_2D_22 ] ) ;
						aPosteriori1[ index_1D_22 ] += ext_info22[ index_2D_22 ] ;
							}
						}
					else
						{
			...
			{
			uint16_t col = 0 ;
			ext_info22[ index_2D_22 ] = trans_info21_22[row] ; 
			//fprintf( stderr, "ext_info22( %d) = %lf \t", row  , ext_info22[ index_2D_22 ] ) ;
			aPosteriori1[ index_1D_22 ] += ext_info22[ index_2D_22  ] ;
			}
		}
		...
		if  ( range > 1) 
				...
				if ( trans_info22_21[row] !=0 )
					...
					for ( col = 0 ; col < range ; col++)
						...
						//fprintf( stderr, "ext_info21( %d,%d ) = %lf \t", row ,col , ext_info21[ index_2D_21 ] ) ;
						aPosteriori1[ index_1D_21 ] += ext_info21[ index_2D_21 ] ;
							}
						}
					else
						{
			...
			{
			uint16_t col = 0 ;
			ext_info21[ index_2D_21 ] = trans_info22_21[row] ; 
			//fprintf( stderr, "ext_info21( %d) = %lf \t", row  , ext_info21[ index_2D_21 ] ) ;
			aPosteriori1[ index_1D_21 ] += ext_info21[ index_2D_21  ] ;
			}
		}
...
void updateMessage12 ()
{
		...
		for ( row = 0 ; row < nrows ; row++ )
			...
			__loop_pipelining_on__(63,2,0);
					...
					//
					//	iterating through all the bit nodes of a check node and updating message.
					message12 [ index_2D_12 ] =  aPosteriori1[ index_1D_12 ] - ext_info12 [ index_2D_12 ] ;
#ifdef SW
...
void updateMessage22 ()
{
		...
		for ( row = 0 ; row < nrows ; row++ )
			...
			__loop_pipelining_on__(63,2,0);
					...
					//
					//	iterating through all the bit nodes of a check node and updating message.
					message22 [ index_2D_22 ] =  aPosteriori1[ index_1D_22 ] - ext_info22 [ index_2D_22 ] ;
#ifdef SW
...
void top_daemon_1 ()
{
	...
	while(nitr_1 < maxNitr)
		...
		write_uint8("aPosteriori1_phase_1_done",0) ;
		...
		uint8_t is_decoded_2 = read_uint8("is_decoded_2") ; 
		
		uint8_t is_decoded = is_decoded_1 & is_decoded_2 ;
...
void top_daemon_2()
{
	...
	{	
		...
		write_uint8("trans_info12_11_calulation_done",0);
			
		uint8_t init_trans_info = read_uint8("trans_info11_12_calulation_done") ;
		
		transverseCorrection12 () ;
		
		fprintf(stderr,"Now! write aPosteriori2_phase_1_done -daemon 2 \n " );
		...
		uint8_t is_decoded_2 = checkIsdecoded2( ) ;
		
		write_uint8("is_decoded_2",is_decoded_2) ;
...
void daemon_3()
{
	...
	uint8_t nitr_3 = 1 ;
		...
		fprintf(stderr, "Buddy! I am wait for - aPosteriori1_phase_1_done \n ");
		
		uint8_t aPosteriori_update_phase1_done = read_uint8("aPosteriori1_phase_1_done") ;
		
		fprintf(stderr ,"aPosteriori1_phase_1_done = %d \n", aPosteriori_update_phase1_done );
...
void daemon_4()
{
	...
	uint8_t nitr_4 = 1 ;
		...
		fprintf(stderr, "Buddy! I am wait for - aPosteriori2_phase_1_done \n");
